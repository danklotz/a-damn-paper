...
```{r}
data_path <- "./data/intervalscore-lense"
out_path <- "./outputs/"
```
...
for (idx in seq(1, length(filelist))) {
  ...
  results_experiment <- dplyr::mutate(results_experiment,
               nse_max = pmax(LENSE_above, metric_below),
               nse_min = pmin(LENSE_above, metric_below),
               interval_score = case_when(LENSE_all < nse_min ~ LENSE_all - nse_min,
                                          LENSE_all > nse_max ~ LENSE_all - nse_max,
                                         TRUE ~ 0.0))
  
  lense_plot <- ggplot(results_experiment) +
    geom_point(aes(x = w, y = LENSE_above, color = interval_score),
               size = 4.5,
               shape = 15) +
    labs(title = "Interval scores",
         subtitle = TeX(paste('(a) $LENSE_{low}$ = ', format(round(metric_below,2), nsmall=2))),
  ...
  ggsave(lense_plot,
         out_path %//% "interval-plot-lense" %-% format(round(metric_below, 2), nsmall = 2) %.% "pdf",
         plot = le_plot,
         dpi = 300,
         height = 9.3,
         width = 8.3,
